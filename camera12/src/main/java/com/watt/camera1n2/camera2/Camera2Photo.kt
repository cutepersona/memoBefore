...
import com.watt.camera1n2.ImageFileViewDialog
...
class Camera2Photo :ConstraintLayout, LifecycleObserver {
    ...
    private final var TAG: String? = "Camera2Photo"
    ...
    private var imageFileViewDialog: ImageFileViewDialog?=null
...
//    private var isCreated: Boolean = false;
    private var pictureStatus: Int = 0;
    ...
    private var zoomLevel:Int by Delegates.observable(1){ _, oldValue, newValue->
        ...
        l.e("Noah zoomLevel zoomLevel:$oldValue newValue:$newValue")
        when(newValue){
            2 -> {
                // 카메라1기준 zoom == 1.5f
                setZoom(1.25f)
    ...
    var callbackRefeshSavePhoto:(()->Unit)? = null
    var callbackOnClickVideo:(()->Unit)? =null


    private var listener: Camera2Photo.onTestListener? = null

    fun setListener(listener1: onTestListener?)
    {
        listener = listener1;
    }
    ...
    {
        fun onClickedBack(){}
        fun onCompleteSavePhoto(uri: String){}
        fun onCanceledSavePhoto(){}
        fun onRefreshSavePhoto(){}
        fun onClickedVideo(){}
    }
    ...
    // Setting Values
    fun setSaveDir(filePath: String?, status: Int){
...
//            }
    ...
    private fun setFlashOn(isOn: Boolean)
    {
        if(isOn){
            isFlashStatus = true
            previewRequestBuilder?.set(
            ...
            captureSession?.setRepeatingRequest(previewRequestBuilder!!.build(), null, null)
            binding.tvFlashStatus.text = resources.getString(R.string.flash_off)
        }
    }
    ...
    private var surfaceTextureListener: TextureView.SurfaceTextureListener = object : TextureView.SurfaceTextureListener {
        ...
        override fun onSurfaceTextureAvailable(p0: SurfaceTexture, width: Int, height: Int) {
            ...
            l.d("onSurfaceTextureAvailable, open camera width:${width} height:$height}")
...
//            else
//                btnText = "확인"
//            imageFileViewDialog = ImageFileViewDialog(context, bitmap, btnText){
//                when(it){
//                    0 -> { // save
//                        saveImage()
//                    }
...
//                    2 -> { // refresh
//                        localBitmap = null
//                        listener?.onRefreshSavePhoto()
//                        callbackRefeshSavePhoto?.let { callback ->
//                            callback()
//                        }
//
//                        resetCam()
...
//            }
            ...
            imageFileViewDialog = ImageFileViewDialog(context, bitmap, btnText, object : ImageFileViewDialog.Listener {
                override fun onClicked(clickedID: Int) {
                    when (clickedID) {
                        0 -> { // save
                            saveImage()
                        }
        ...
        imageFileViewDialog?.setOnDismissListener {
            imageFileViewDialog = null
            bitmap.recycle()
        }

        imageFileViewDialog?.let{
            if(!it.isShowing)
                it.show()
        }
    }
    ...
    private fun onLaunchDictation(){
        ...
        try {
            ...
            listener?.onCompleteSavePhoto(uri)
...
//            }
            ...
            l.d("Noah previewSize w ${previewSize!!.width} h ${previewSize!!.height} rotatedPreviewWidth $rotatedPreviewWidth rotatedPreviewHeight $rotatedPreviewHeight --- view w:$width h:$height")
...
//            previewSize!!.width.toFloat()
        ...
        l.d("Noah configureTransform rotation : $rotation")
...
//        }
    ...
    private fun openCamera(width: Int, height: Int) {
        l.d("Noah openCamera() : width:$width height:$height")
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        override fun onOpened(camera: CameraDevice) {
            l.d("Noah stateCallback : onOpened - zoomLevel$zoomLevel")
        ...
        override fun onDisconnected(camera: CameraDevice) {
            l.d("Noah stateCallback : onDisconnected")
            cameraOpenCloseLock.release()
        ...
        private fun process(result: CaptureResult) {
            when (state) {
                ...
                STATE_WAITING_LOCK -> {
                    ...
                    Log.d(TAG, "Noah STATE_WAITING_LOCK - " + afState);
                    if (afState == null) {
                        captureStillPicture()
                        ...
                        Log.d(TAG, "Noah STATE_WAITING_LOCK CONTROL_AE_STATE - " + afState);
                        if (aeState == null ||
                ...
                STATE_WAITING_PRECAPTURE -> {
                    ...
                    Log.d(TAG, "Noah STATE_WAITING_PRECAPTURE");
                    // CONTROL_AE_STATE can be null on some devices
                    val aeState = result.get(CaptureResult.CONTROL_AE_STATE)
                ...
                STATE_WAITING_NON_PRECAPTURE -> {
                    ...
                    Log.d(TAG, "Noah STATE_WAITING_NON_PRECAPTURE");
                    // CONTROL_AE_STATE can be null on some devices
                    val aeState = result.get(CaptureResult.CONTROL_AE_STATE)
...
//            if (zoom != null) previewRequestBuilder!!.set(CaptureRequest.SCALER_CROP_REGION, zoom)
    ...
    private fun createCameraPreviewSession() {
        try {
            ...
            l.d("Noah image dimension width:${previewSize?.width}, height:${previewSize?.height}")
    ...
    private val mCameraBroadcastReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent) {
            if (intent.action == INTENT_GET_CAMERA_SETTINGS) {
                ...
                l.d("Noah sensor:$sensor, eis:$eis, fov:$fov")
...
//        zoomLevel = 1
        l.d("Noah setTelephoto " + zoomLevel)
...
//        zoomLevel =1
    ...
    private fun setFovOFF()
    {
        l.d("Noah setFovOFF")
        val intent = Intent(INTENT_SET_CAMERA_SETTINGS)
        intent.putExtra(EXTRA_FOV, fov_wide)
        intent.component = ComponentName(ANDROID_SETTINGS_PKG, RW_CAMERA_SET_RECEIVER)
        context.sendBroadcast(intent)
    }
